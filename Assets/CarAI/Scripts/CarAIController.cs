...
public class CarAIController : MonoBehaviour
{
    ...
    private void SearchForCheckpoints()
    {
        if (CheckPointSearch && isCarControlledByAI)
        {
            ...
            bool searchForOtherObjects = Physics.Raycast(check.position, check.forward, out carHit, maxDistance, Physics.AllLayers, QueryTriggerInteraction.Ignore);
            UnityEngine.Debug.DrawRay(check.position, check.forward * maxDistance, Color.green);

            if (searchForOtherObjects && carHit.transform != transform)
            {
                // Object detected, check its tag
                if (carHit.transform.CompareTag("Checkpoint"))
                {
                    // Checkpoint detected, maintain or accelerate to the speed limit
                    SetSpeed(speedLimit);
                }
                else
                {
                    // Other object detected, apply braking torque
                    Break(breaking);
                    audioSource.Play();
                }
                objectDetected = true;
            ...
            {
                // No object detected, maintain or accelerate to the speed limit
                SetSpeed(speedLimit);
